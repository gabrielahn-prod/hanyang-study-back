...
`Google ADK` 기반의 로컬 테스트용 멀티 에이전트 예제입니다. 현재 저장소는 `env_agent` 루트 에이전트가 4개의 서브에이전트(`time_agent`, `runtime_agent`, `everytime_agent`, `general_agent`)를 오케스트레이션하고, 이를 `FastAPI` 또는 `adk web`으로 실행할 수 있게 구성되어 있습니다.
...
EVERYTIME_MCP_URL=http://everytime-mcp:8001/mcp
EVERYTIME_MCP_TIMEOUT=10
```
...
- `agents/env_agent/everytime_agent.py`: Everytime MCP 연결용 서브에이전트 정의
...
Everytime 조회는 `everytime_agent`가 외부 MCP 서버에 연결해서 처리합니다.
...
- 현재 연결 대상 MCP는 읽기 전용 조회 용도입니다.
- 지원 범위는 홈 페이지 요약, 게시판 목록, 특정 게시판 글 목록, 게시판 디버그 조회입니다.
- 미지원 범위는 글 본문 단건 조회, 댓글 조회, 로그인 자동화, 쿠키 자동 갱신입니다.
- 기본 tool 매핑은 `get_home_summary`, `list_boards`, `get_board_posts`, `debug_board`, `health_check`입니다.
- 같은 compose 네트워크 안에서는 서비스명 `everytime-mcp`로 접근하는 구성을 권장합니다.
- 기본 포트 예시는 ADK 서버 `8000`, Everytime MCP `8001`입니다.
...
- `everytime_agent`: Everytime MCP 기반 홈/게시판 조회 요청 처리
