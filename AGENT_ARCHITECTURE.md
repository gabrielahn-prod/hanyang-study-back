...
이 저장소의 아키텍처는 `Google ADK` 런타임 위에 `env_agent` 루트 에이전트를 올리고, 그 아래에 3개의 역할별 서브에이전트를 둔 뒤, `FastAPI`로 이를 서빙하는 구조입니다. 모델 공급자를 환경 변수로 바꾸는 실행 환경 중심 설계는 유지하면서, 간단한 멀티 에이전트 오케스트레이션을 추가했습니다.
...
Client / ADK Web / curl
 ...
 get_fast_api_app(...)
   ...
   |                |             |
   v                v             v
time_agent     runtime_agent   general_agent
   |                |
   v                v
get_local_time()  get_runtime_config()
        |
        v
   Model Resolver
   build_model()
...
### [`app.py`](/Users/ahngab/Desktop/adk-examples/app.py)
...
### [`agents/env_agent/agent.py`](/Users/ahngab/Desktop/adk-examples/agents/env_agent/agent.py)

- 실제 에이전트 정의가 있는 핵심 파일입니다.
- `root_agent = Agent(...)` 형태로 ADK가 읽을 루트 에이전트를 선언합니다.
- 루트 에이전트 아래에 3개의 서브에이전트를 둡니다.
...
### [`agents/agent.py`](/Users/ahngab/Desktop/adk-examples/agents/agent.py)
...
2. [`app.py`](/Users/ahngab/Desktop/adk-examples/app.py)에서 생성한 ADK FastAPI 앱이 요청을 받습니다.
...
6. 일반 요청은 `general_agent`가 처리합니다.
7. 최종 응답은 다시 FastAPI를 통해 클라이언트로 반환됩니다.
...
이 값들은 [`app.py`](/Users/ahngab/Desktop/adk-examples/app.py)에서 `get_fast_api_app(...)`로 전달됩니다. 따라서 애플리케이션 코드가 상태 저장소를 직접 다루지 않고, 인프라 설정으로 세션/메모리 저장소를 교체할 수 있습니다.
...
이 프로젝트는 "FastAPI가 ADK 런타임을 감싸고, ADK 런타임이 `env_agent` 루트 에이전트를 로드하며, 루트가 3개의 서브에이전트에 요청을 위임하고, 각 에이전트가 환경 변수에 따라 Gemini 또는 OpenAI 모델과 로컬 툴을 사용해 응답하는 구조"입니다.
